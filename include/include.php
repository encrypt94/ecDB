		...
		if(isset($_POST['submit']) or isset($_POST['update'])) {
			...
			elseif (!empty($_POST['height']) && !is_numeric($height)) {
				...
				if(isset($_POST['submit'])) {
					$sql="INSERT into data (owner, name, manufacturer, package, pins, smd, quantity, location, scrap, width, height, depth, weight, datasheet, comment, category, url1, url2, url3, url4, price, order_quantity, bin_location, public)
					VALUES
					('$owner', '$name', '$manufacturer', '$package', '$pins', '$smd', '$quantity', '$location', '$scrap', '$width', '$height', '$depth', '$weight', '$datasheet', '$comment', '$category', '$url1', '$url2', '$url3', '$url4', '$price', '$order_quantity', '$bin_location', '$public')";
